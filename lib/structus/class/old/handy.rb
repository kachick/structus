module Structura::Class
  ...
  def to_struct_class
    ...
    if name
      ...
      if ::Structura::Structs.const_defined?(tail_name) && 
          ((already = ::Structura::Structs.const_get(tail_name)).members == members)
          already
      else
        ::Structura::Structs.const_set tail_name, struct_klass
      end
    else
      struct_klass
    end
